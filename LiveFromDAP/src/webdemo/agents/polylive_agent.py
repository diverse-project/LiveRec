...
from prettyprinter.LivExPrettyPrinter import LivExPrettyPrinter
from .base import BaseAutoLiveAgent

class AutoPolyLiveAgent(BaseAutoLiveAgent):
    def __init__(self, raw=False):
        super().__init__(raw)
        self.agent = PolyglotLivExAgent(debug=False)
    ...
    def execute(self, method, args, probes):
        ...
        if output[0] == "Interrupted":
            self.agent.load_code(self.source_path)
        return self.construct_result_json(method, output, LivExPrettyPrinter)