...
sequenceDiagram
    participant A as Live Probes server
    participant B as Debugger (KAA)
    A->>B: Execute method(args)
    B->>A: Breakpoint at method entry
    loop Until method end
        A->>B: request stackframe
        B->>A: stackframe
        A->>B: request step
        B->>A: hit breakpoint
    end
