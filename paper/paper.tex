...
\textbf{TODO: FIX THIS:}
The basic intuition of our approach to meet these requirements is as follows: the editor starts a debugger, not on the user program, but on a driver program called the Keep Alive Agent (KAA). This program consists of a while-true-loop, halted on a breakpoint, and is designed to service live probe requests, without having to restart the debugger (that would be too slow). When the code changes
